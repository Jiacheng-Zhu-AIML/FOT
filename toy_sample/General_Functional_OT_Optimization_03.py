...
03:
Balanced, two domain have potentially different density
...
from General_Functional_OT_Optimization import GFOT_optimization, loss_l2_average, loss_weighted_l2_average
...
if __name__ == '__main__':
    ...
    y_12 = 6 * np.sin(0.15 * (x + 5.5)) - x / 10 - 3.7
    y_13 = 6 * np.sin(0.15 * (x + 5.5)) - x / 10 - 3.8
    y_14 = 6 * np.sin(0.15 * (x + 5.5)) - x / 10 - 3.9
    y_15 = 6 * np.sin(0.15 * (x + 5.5)) - x / 10 - 4.7
    l_num = 5
    F1_list = [y_11, y_12, y_13, y_14, y_15]

    y_21 = 6 * np.sin(0.15 * (x + 1.7)) + x / 15 - 3.0
    y_22 = 6 * np.sin(0.15 * (x + 1.7)) - x / 15 - 3.6
    y_23 = 6 * np.sin(0.15 * (x + 1.7)) - x / 15 - 3.9
    y_24 = 6 * np.sin(0.15 * (x + 1.7)) - x / 15 - 4.2
    y_25 = 6 * np.sin(0.15 * (x + 1.7)) - x / 15 - 4.5


    k_num = 5
    F2_list = [y_21, y_22, y_23, y_24, y_25]

    # Notice: Plot it in 3D
    # fig = plt.figure()
    ...
    X1_train = np.concatenate([x, x, x, x, x], axis=0)
    Y1_train = np.concatenate([y_11, y_12, y_13, y_14, y_15], axis=0)

    X2_train = np.concatenate([x, x, x, x, x], axis=0)
    Y2_train = np.concatenate([y_21, y_22, y_23, y_24, y_25], axis=0)
    ...
    ini_A = np.eye(10)
    ini_Pi = 1.0 / 5 * np.ones((l_num, k_num))
    ...
    A_mat, Pi = GFOT_optimizer.Optimize(lr_A=lr_A, lr_Pi=lr_Pi, tho=1e-5,
                                        ...
                                        inequality=False)
    # Notice: Condut the GFOT
    f_sharp_11 = V @ A_mat @ U.T @ y_11
    f_sharp_12 = V @ A_mat @ U.T @ y_12
    f_sharp_13 = V @ A_mat @ U.T @ y_13
    f_sharp_14 = V @ A_mat @ U.T @ y_14
    f_sharp_15 = V @ A_mat @ U.T @ y_15
    f_sharp_list = [f_sharp_11, f_sharp_12, f_sharp_13, f_sharp_14, f_sharp_15]
    ...
    f_shp_14_gpot, _ = expmap(y_14, K_1, v_mu, v_T)
    f_shp_15_gpot, _ = expmap(y_15, K_1, v_mu, v_T)

    f_gp_shp_list = [f_shp_11_gpot, f_shp_12_gpot,
                     f_shp_13_gpot, f_shp_14_gpot,
                     f_shp_15_gpot]

    # Notice: Compare
    print('np.sum(Pi) =', np.sum(Pi))
    gfot_loss = loss_weighted_l2_average(F2_list, f_sharp_list, Pi * 5 / np.sum(Pi))
    print('gfot_loss =', gfot_loss)

    gpot_coupling = np.ones((l_num, k_num)) / l_num
    gpot_loss = loss_weighted_l2_average(F2_list, f_gp_shp_list, gpot_coupling)
    print('gpot_loss =', gpot_loss)
    ...
    # ax.scatter3D(layer_2, X_test, f_sharp_14, c='orange', label='T#y_14', alpha=0.3)
    # ax.scatter3D(layer_2, X_test, f_sharp_15, c='orange', label='T#y_15', alpha=0.3)
    #
    ...
    #         alpha_3d = Pi[l, k]
    #         ax.plot3D(layer_3d, X_test_3d, f_3d, c=(0.8, l*(1.0/l_num) , 1 - l*(1.0/l_num)), alpha=alpha_3d)
    #
    # plt.legend()
    ...
    plot_origin_domain_data(plt, x, F1_list, marker='+',
                            color='r', label='F1, source')
    ...
    plot_origin_domain_data(plt, x, F2_list, marker='x',
                            color='b', label='F2, target')
    ...
    plt.plot(X_test, f_sharp_14, c='orange', label='FOT T#y_14', alpha=0.9)
    plt.plot(X_test, f_sharp_15, c='orange', label='FOT T#y_15', alpha=0.9)

    # notice: The mapped data by GPOT

    plt.plot(X_test, f_shp_11_gpot, c='lime', label='GPOT T#Y', alpha=0.4)
    plt.plot(X_test, f_shp_12_gpot, c='lime', alpha=0.4)
    plt.plot(X_test, f_shp_13_gpot, c='lime',  alpha=0.4)
    plt.plot(X_test, f_shp_14_gpot, c='lime',  alpha=0.4)
    plt.plot(X_test, f_shp_15_gpot, c='lime',  alpha=0.4)
